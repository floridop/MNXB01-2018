...
#
# Data structure format is an array of objects in JSON data format.
...
wget -P $FTTEMPDIR http://fortunecookieapi.herokuapp.com/v1/fortunes &>/dev/null

# E3 (1 point)
# use the dumpfile function to print the content of the fortunes file.
# print option is --printinfo
...
if [[ "${PRINTINFO}" == "--printinfo" ]]; then
    echo "Exiting because --printinfo specified"
    exit 0
elif [[ "${CLEANUP}" == "--cleanup" ]]; then
    cleanup $FTTEMPDIR
fi
...
NUMMSG=`cut -c1 $FTTEMPDIR/fortunes_messages | wc -l`
#echo $NUMMSG
...
CHOSEN=`echo $(($RANDOM%$NUMMSG + 1)) | bc`
#echo $CHOSEN
...
MESSAGE=`head -n $CHOSEN $FTTEMPDIR/fortunes_messages | tail -n 1` | cut -d':' -f2
