...
#
# Datastructure format is JSON [{"message":"Fortune","id":"id-number"},{"message":"Fortune","id":"id-number"},...].
...
mktemp -d /tmp/ftellerXXX
FTTEMPDIR=$ftellerXXX
...
cat /tmp/$FTTEMPDIR/fortunes | json_pp -f json -t dumper > /tmp/$FTTEMPDIR/fortunes_pp

# E5 (1 points)
# use the dumpfile function to print the content of the fortunes_pp file.
dumpfile /tmp/$FTTEMPDIR/fortunes_pp
...
grep "message" /tmp/$FTTEMPDIR/fortunes_pp > /tmp/$FTTEMPDIR/fortunes_messages
...
NUMMSG=$(wc -l /tmp/$FTTEMPDIR/fortunes_messages | cut -d' ' -f1)
...
MESSAGE=$(head -n $CHOSEN /tmp/$FTTEMPDIR/fortunes_messages | tail -n +$CHOSEN)
echo -e cat $MESSAGE | cut -d ">" -f 2
