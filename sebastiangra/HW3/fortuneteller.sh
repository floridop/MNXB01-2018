...
#
# Datastructure format is _________JSON___________
...
#<YOUR CODE HERE>
FTTEMPDIR=$(find . -name fteller??? -type d -print | head -n1) 
...
#<YOUR CODE HERE>
wget http://fortunecookieapi.herokuapp.com/v1/fortunes -O $FTTEMPDIR/.fortunes 
...
#<YOUR CODE HERE>
if [[ "${PRINTINFO}x" == "--printinfox" ]]
then
    echo "Existing because --printinfo specified"
    exit 0
fi
if [[ "${CLEANUP}x" == "--cleanupx" ]]
then
    cleanup $FTTEMPDIR
fi
