...
#<YOUR CODE HERE>
FTTEMPDIR=$(mktemp -d -p /tmp ftellerXXX)
...
#<YOUR CODE HERE>
#This file print is not shown in the call_with_printinfo_output template.
#call_with_printinfo_output shows fortunes_pp only, but says it is fortunes.
...
#To match the call_with_cleanup_output the cleanup needs to be in the 
#end of the file, so I added the code there.

# E7 (1 point)
# Extract only the lines containing "message" and save them as $FTTEMPDIR/fortunes_messages
#<YOUR CODE HERE>
cat $FTTEMPDIR/fortunes_pp | grep "message" > $FTTEMPDIR/fortunes_messages
...
#<YOUR CODE HERE>
NUMMSG=$(wc -l $FTTEMPDIR/fortunes_messages | cut -d' ' -f1)
...
#<YOUR CODE HERE>
MESSAGE_CONTENT=$(echo "$MESSAGE" | cut -d'"' -f4)
...
#cleanup according to task E6 to match call_with_cleanup_output.
if [[ "${CLEANUP}x" == "--cleanupx" ]]; then
        cleanup $FTTEMPDIR
fi
